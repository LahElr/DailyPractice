...
class Graph:
    def __init__(self,arg:Union[Iterable[Iterable[Any]],int],default_val = 1,directional = True):
        ...
        if isinstance(arg,int):
            ...
            for edge in edges:
                ...
                try:
                    ...
                    end_id = node_count
                    node_count+=1
                edge_ids.append([start_id,end_id,edge[2] if len(edge)>=3 else default_val])
    ...
    def set_edge(self,start,end,val = None):
        try:
            self.matrix[self.name_key_bidict[start]][self.name_key_bidict[end]] = val if val is not None else self.default_val
            if not self.directional:
                self.matrix[self.name_key_bidict[end]][self.name_key_bidict[start]] = val if val is not None else self.default_val
        except KeyError:
    ...
    def get_in_degree(self,node,except_node_list=[]):
        return sum([self.matrix[self.name_key_bidict[_]][self.name_key_bidict[node]] for _ in self.name_key_bidict.keys() if _ not in except_node_list])
    
    def get_out_degree(self,node,except_node_list=[]):
        return sum([self.matrix[self.name_key_bidict[node]][self.name_key_bidict[_]] for _ in self.name_key_bidict.keys() if _ not in except_node_list])
    
    def __set_edge(self,start_id,end_id,val = None):
        try:
            self.matrix[start_id][end_id] = val if val is not None else self.default_val
            if not self.directional:
                self.matrix[end_id][start_id] = val if val is not None else self.default_val
        except IndexError:
    ...
    def __get_in_degree(self,node_id,except_node_list=[]):
        try:
            return sum([self.matrix[_][node_id] for _ in range(self.node_count) if _ not in except_node_list])
        except IndexError:
            raise ValueError("Not exist node id",stacklevel=2)
    
    def __get_out_degree(self,node_id,except_node_list=[]):
        try:
            return sum([self.matrix[node_id][_] for _ in range(self.node_count) if _ not in except_node_list])
        except IndexError:
            raise ValueError("Not exist node id",stacklevel=2)
...
x = Graph([[1,2],[1,4],[2,4],[4,3],[5,4],[3,5],[2,3]])
print(x.topology_sort())
